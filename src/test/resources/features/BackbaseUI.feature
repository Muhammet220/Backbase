...
Feature:CRUD Articles
  ...
  Scenario Outline:Create a new article
    ...
    Examples:
      ...
      | GrandMaster | Anish Giri                | Anish Kumar Giri is a Russian-born Dutch chess grandmaster | None |

  Scenario:Update article
    When the user clicks created article
    And the user clicks Edit Article button
    And the user changes Article Title as "Chess Grandmaster"
    And the user clicks Publish Article button
    Then the user should  be able to  update Article Title as "Chess Grandmaster"
